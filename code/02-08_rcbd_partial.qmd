...
```{r rcbd_dfw}
rcbd_dfw <- rcbd_df %>%
  mutate(rep = factor(rep), #you do not want your rep/block to be read as dbl by R, it will give you an error
         nrate_kgha = factor(nrate_kgha),
         krate_kgha = factor(krate_kgha) 
         ) %>%
  mutate(trtname =
           paste0(nrate_kgha,"+",krate_kgha))
...
rcbd_mod <- lm(yield_kgha ~ rep + nrate_kgha*krate_kgha,
               data = rcbd_dfw
               ) #noise, soil variability, is removed or reduced by rep
...
```{r ANOVA}
Anova(rcbd_mod, type=3) #reps - we constrained randomization by the blocks/reps; NOTE: discuss the interaction first, then you should discuss the interacton first; type = 3, means 3-way ANOVA
```
...
```
Looks great! Next.  
  *residuals change depending on the model
...
-* normalizing residuals will is beneficial to check for residual outliers
...
rcbd_cld_all <- cld(rcbd_means_all, 
                   reversed=T, 
                   adjust="none",
                   #adjust = "tukey" #more comparisons, then we have higher chances of false positive; thus, we need to be more strict by changing the adjustment and changing the alpha - MOST strict = adjust = "tukey"; alpha = 0.05
               Letters=letters,
               alpha = 0.05)
...
mutate(letter = trimws(.group)) %>%
  mutate(trtname = paste0(nrate_kgha, "+",
                          krate_kgha
                          ))
rcbd_cld_selected
```
...
ggsave("../output/rcbd_means.png", width = 8, height = 5) 
