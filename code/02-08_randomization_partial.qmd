...
trts <- crossing(nrate_kgha = c(0,100,200),
  ...
  mutate(trtnum = 1:nrow(.)) %>% #telling it to do a sequence from 1-9 (nrow(.) = number of rows)
...
set.seed(930) #makes randomization reproducible
...
mutate(plot = sample(plot)) #shuffle - sample fn
...
geom_tile(aes(fill = trtname),
          color = "black",
          show.legend = F) +
  geom_text(aes(label = lab), vjust = 0.5) + #vjust = vertical justification of number
  labs(title = "CRD randomization of a 3 x 3 factoral (9 treatments)") +
  theme_void()
...
group_by(rep) %>%
  ...
  mutate(plot = (rep*100) + 1:9) %>% #arrange by rep because we need to randomize by rep
  ungroup()
...
left_join(trts) %>% #left_join fn - merge two datasets with the same column name and column values - join from right side to left side
  # Rearranging columns  
